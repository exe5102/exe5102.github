...
def ClientEdit(uphone,pnumber,unumber,stime,State):                  # 修改訂位內容    不正常
    try:
        conn = sqlite3.connect('Booking.db') 
...
def RecordLogin(number,pnumber,intime,outtime,amount,Note):                    # 車牌號碼 車位號碼 進場時間 離場時間 金額
    try:
        conn = sqlite3.connect('Booking.db') 
        conn.execute( f"insert into Record(Number , Pnumber, InTime, OutTime, Amount, Note)\
                            select '{number}', '{pnumber}', '{intime}' ,'{outtime}','{amount}','{Note}' \
                        where not exists(\
                            select 1 from Record where Number='{number}' and InTime='{intime}' and OutTime='{outtime}' and Amount='{amount}'and Note='{Note}');")
        conn.commit()
        conn.close()
    except Exception as e:
        print(f'=>資料庫連接或資料表建立失敗，錯誤訊息為{e}')
